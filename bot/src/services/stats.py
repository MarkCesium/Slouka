...
class StatsService:
    ...
    async def log_review(self, user_id: int, card_id: int, quality: int) -> None:
        async with self._uow:
            await self._uow.review_logs.log_review(user_id, card_id, quality)

    async def get_calendar_data(self, user_id: int, year: int, month: int, tz: str) -> set[int]:
        async with self._uow:
            return await self._uow.review_logs.get_active_days(user_id, year, month, tz)
    ...
    async def get_review_counts(self, user_id: int, tz: str) -> dict[str, int]:
        ...
        async with self._uow:
            week = await self._uow.review_logs.count_reviews_in_range(
                user_id, week_start_utc, now_utc
            )
            month = await self._uow.review_logs.count_reviews_in_range(
